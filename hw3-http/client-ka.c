...
void server_disconnect(int sock) { close(sock); }
...
int submit_request(int sock, const char *host, uint16_t port, char *resource) {
  ...
  if (sent_bytes < 0) {
    ...
    // 1.  Use the sock = reopen_socket() helper you implemented above in an
    // attempt
    //     to reopen_the socket.  If it returns a value less than zero, a socket
    //     could not be created so return the negative value and exit the
    //     function, e.g., return sock
    //
    ...
    if (sock < 0) {
      fprintf(stderr, "Failed to reopen socket after retries\n");
      return -1;
    }
  ...
  int bytes_recvd =
      0; // used to track amount of data received on each recv() call
  int total_bytes =
      0; // used to accumulate the total number of bytes across all recv() calls
  ...
  // remember the first receive we just did has the HTTP header, and likely some
  // body data.  We need to determine how much data we expect

  //--------------------------------------------------------------------------------
  // TODO:  Get the header len
  //
  // 1. Use the get_http_header_len() function to set the header_len variable.
  // 2. The get_http_header_len() function returns a negative value if it fails,
  // so
  ...
  if (res != 0) {
    fprintf(stderr, "Failed to retrieve HTTP header and content length\n");
    close(sock);
    return -1;
  }
  ...
  //
  // Note that no error checking is needed, if the get_http_content_len()
  // function cannot find a Content-Length header, its assumed as per the HTTP
  // spec that ther is no body, AKA, content_len is zero;
  //--------------------------------------------------------------------------------
  ...
  // defensive an prevent an infinite loop, i have it set to keep looping as
  // long as bytes_remaining is positive
  ...
  // TODO:  Documentation
  //
  // You dont have any code to change, but explain why this function, if it gets
  // to this point returns an active socket.
  //
  // YOUR ANSWER:
...
int main(int argc, char *argv[]) {
  ...
  // YOU DONT NEED TO DO ANYTHING OR MODIFY ANYTHING IN MAIN().  MAKE SURE YOU
  ...
  if (argc < 4) {
    ...
    if (port == 0) {
      fprintf(stderr,
              "NOTE: <port> must be an integer, using default port %d\n",
              DEFAULT_PORT);
      port = DEFAULT_PORT;
    }
