...
course_item_t *lookup_course_by_id(char *course_id) {
  static course_item_t NOT_FOUND_COURSE = {"NONE",
...
static void process_requests(int listen_socket) {
  ...
  while (1) {
     ...
     *
     *      Here is some helper code after you receive data from the client.
     * This helps get setup to actually process the client request
     *
     *      cs472_proto_header_t *pcktPointer =  (cs472_proto_header_t
     * *)recv_buffer; uint8_t *msgPointer = NULL; uint8_t msgLen = 0;
     *      process_recv_packet(pcktPointer, recv_buffer, &msgPointer, &msgLen);
     *
     */

    // TODO:  DELETE THESE VARIABLES BELOW...
    // SEE THE COMMENT ABOVE, THESE VARIABLES ARE JUST PUT IN HERE FOR NOW TO
    // MAKE SURE THE STUB COMPILES
    ...
    // Process the received packet, extract message pointer and length
    process_recv_packet(pcktPointer, recv_buffer, &msgPointer, &msgLen);

    // Now lets setup to process the request and send a reply, create a copy of
    // the header also switch header direction
    memcpy(&header, pcktPointer,
           sizeof(cs472_proto_header_t)); // start building rsp header
    header.dir = DIR_RECV;
    switch (header.cmd) {
    case CMD_CLASS_INFO:
      ...
      prepare_req_packet(&header, (uint8_t *)details->description,
                         strlen(details->description), send_buffer,
                         sizeof(send_buffer));
      break;
    case CMD_PING_PONG:
      ...
      prepare_req_packet(&header, (uint8_t *)msg_out_buffer,
                         strlen(msg_out_buffer), send_buffer,
                         sizeof(send_buffer));
      break;
    default:
